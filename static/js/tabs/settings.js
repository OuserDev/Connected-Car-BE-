...
export function renderSettings(){
  ...
  if (token) {
    baseCard.querySelector("#btnLogout")?.addEventListener("click", ()=>{
      State.setToken(null); State.setUser(null);
      UI.toast("로그아웃 되었습니다."); renderSettings();
    });
...
const photoCard = token ? (() => {
  ...
  function renderGallery(){
    const u = getUser();
    const photos = Array.isArray(u.carPhotos) ? u.carPhotos : [];
    // hero
    if (u.carPhotoData) {
      $hero.innerHTML = `<img class="hero-img" src="${u.carPhotoData}" alt="메인 차량 사진">`;
    } else if (photos.length){
      $hero.innerHTML = `<img class="hero-img" src="${photos[0].dataUrl}" alt="메인 차량 사진 후보">`;
    } else {
    ...
    photos
      ...
      .forEach(p=>{
        const isMain = u.carPhotoData && u.carPhotoData === p.dataUrl;
        const el = document.createElement("div");
        el.className = "product-card";
        el.innerHTML = `
          <div class="thumb" style="position:relative; aspect-ratio:1; overflow:hidden; border-radius:10px;">
            <img src="${p.dataUrl}" alt="사진" style="width:100%; height:100%; object-fit:cover;">
          ...
          <div class="muted" style="font-size:12px; margin-top:4px;">${new Date(p.createdAt).toLocaleString()}</div>
        `;
        $grid.appendChild(el);
      });
  }
  ...
  async function addPhotosFromFiles(fileList){
    ...
    for (const file of Array.from(fileList || [])) {
      ...
      try{
        ...
        // 검증 6: 중복 방지
        if (isDup(photos, dataUrl)) { UI.toast("이미 추가된 사진입니다."); continue; }

        // 저장
        const id = "p_" + Date.now() + "_" + Math.random().toString(36).slice(2,6);
        photos.push({ id, dataUrl, createdAt: Date.now() });
        added++;
    ...
    // 적용
    saveUser({ ...u, carPhotos: photos });
    if (added > 0) UI.toast(`${added}장의 사진을 추가했습니다.`);
    renderGallery();
  }
  ...
  $grid.addEventListener("click", (e)=>{
    ...
    if (act === "setMain"){
      // 메인으로 설정
      const next = photos[idx].dataUrl;
      saveUser({ ...u, carPhotoData: next });
      UI.toast("메인 사진이 변경되었습니다.");
      renderGallery();
    }
    else if (act === "delete"){
      // 삭제
      ...
      photos.splice(idx, 1);
      let nextMain = u.carPhotoData;
      if (u.carPhotoData === target.dataUrl){
        // 메인을 지웠다면 대체(남은 첫 장 or null)
        nextMain = photos[0]?.dataUrl || null;
      }
      saveUser({ ...u, carPhotos: photos, carPhotoData: nextMain });
      UI.toast("사진을 삭제했습니다.");
      renderGallery();
    }
  });
  ...
  $btnClear?.addEventListener("click", ()=>{
    const u = getUser();
    if (!Array.isArray(u.carPhotos) || u.carPhotos.length === 0){
      UI.toast("삭제할 사진이 없습니다.");
      return;
    }
    saveUser({ ...u, carPhotos: [], carPhotoData: null });
    UI.toast("모든 사진을 삭제했습니다.");
    renderGallery();
  });
